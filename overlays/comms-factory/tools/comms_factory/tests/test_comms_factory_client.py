...
def test_ground_from_tools_posts_service_compatible_contract_without_token_value(
    monkeypatch,
):
    calls = []
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        assert body["tool_plane"]["idempotency_prefix"] == "run_1:ground:1"
        assert "capability_plane" not in body
    ...
    result = CommsFactoryClient("http://comms.test", "token").ground_from_tools(
        "brief",
        run_id="run_1",
        idempotency_prefix="run_1:ground:1",
    )
...
def test_generate_applies_defaults_and_maps_5xx(monkeypatch):
    ...
    result = CommsFactoryClient("http://comms.test", "token").generate(
        {"kind": "launch-tier", "deployed_facts": ["Fact A"]}
    )
