...
export function buildGrounderSystemPrompt(
  ...
  lines.push(
    ...
    "   - **DO NOT guess deep doc paths.** Hand-writing a slug like `/docs/ach-and-wire-transfers` is a coin-flip that 404s. DISCOVER the real URL:",
    "     a. Fetch `<docs-domain>/llms.txt` (and if it looks truncated or doesn't list your topic, `/llms-full.txt`) — most doc sites publish a single-file index of the REAL URLs built for exactly this.",
    "     b. Else fetch the docs ROOT and read its nav.",
    "   - **HARD RULE: once you have an index (llms.txt or a fetched root/landing page), you have the REAL link list. Copy the EXACT URL it lists for your topic and fetch THAT. NEVER hand-construct a path after you've seen an index — that is the #1 cause of 404 loops.** If your topic isn't in the truncated llms.txt, fetch `/llms-full.txt` before giving up.",
    "   - Don't assume a `/docs/` prefix or any path shape — real doc paths vary wildly by site (`/platform/...`, `/guides/...`, `/reference/...`, bare slugs, deep nesting). Take the path VERBATIM from the index; never infer or normalize it.",
    "   - Only construct a URL yourself if you can derive it from a path pattern you've ALREADY confirmed works (e.g. fetched `/v2/payments` → `/v2/refunds` is fair).",
    "   - Stripe crypto onramp: start at https://docs.stripe.com/crypto and follow the listed links.",
    ...
    "",
    "**Hard rule: never retry the SAME failing URL/tool input. BUT a 404 on a guessed doc path is NOT 'unverifiable' — it means you guessed wrong. Before marking anything unverifiable, run discovery (llms.txt → docs root → follow nav) to find the real page. Mark unverifiable only once discovery is genuinely exhausted, not after a single 404.**",
    "",
    ...
    "**Source authority for a PARTNER's external behavior** (settlement/clearing times, SLAs, fees, limits, cutoffs, supported regions, processing windows): the PARTNER's OWN docs are authoritative. Our platform code / UI copy is a DERIVATIVE restatement that may be stale or aspirational — it 'knows no better' than the partner's docs do. So do NOT record a partner-behavior fact at 0.9+ just because it came from our code: record it at ≤0.6, tag the value '(per our UI copy — verify against <partner>)', and keep trying the partner's docs (llms.txt → the EXACT listed URL) before settling. Our code IS authoritative for OUR OWN behavior (feature flags, flows, what we display, which provider we use) — just not for what the partner's rails actually do.",
    "",
