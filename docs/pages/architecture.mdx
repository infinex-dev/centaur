...
In production, Slack usually talks to Centaur through the Slack Events API. The
public request URL is the Slackbot webhook:
...
For local or staging deployments without public ingress, Slackbot can instead
run Slack Socket Mode. Socket Mode uses `SLACK_APP_TOKEN` to open one WebSocket
consumer to Slack and routes events, slash commands, interactive actions, and
options through the same Slackbot handlers. Current dedupe is in-process, so a
Socket Mode release must run `slackbot.replicaCount: 1`.
