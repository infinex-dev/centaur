...
export SLACK_APP_TOKEN=...   # xapp- token with connections:write for local/dev Socket Mode
...
for `SLACK_SIGNING_SECRET`. Local/dev values enable Slack Socket Mode so the
bot can receive events without a public Request URL; create an app-level token
with `connections:write` and store it as `SLACK_APP_TOKEN`.
...
`SLACK_APP_TOKEN`, and `SLACKBOT_API_KEY` are API boot requirements in the
current local bootstrap. `SLACK_BOT_TOKEN` is required because Slackbot is
