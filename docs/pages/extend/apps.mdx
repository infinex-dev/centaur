...
Do not confuse this WIP Apps design with `attachedServices`. Attached services
exist today as a low-level Helm primitive for running an internal Deployment and
ClusterIP Service beside the API; they do not provide app manifests, app routes,
registration APIs, lifecycle records, or web-surface proxying.
