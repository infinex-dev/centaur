...
How a separate-service agent (one that runs outside Centaur's runtime, like an
attached service) gets read-only access to Centaur-owned tools.
...
SCOPE_BUNDLES = {
    "research": (
        "tools:repo_context",
        "tools:websearch",      # Exa; page fetches go through its `search` method
        "tools:twitter",
