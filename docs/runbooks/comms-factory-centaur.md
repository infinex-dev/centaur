...
6. Mounts `ANTHROPIC_API_KEY` into the attached comms-factory service when present in `centaur-infra-env`; `ground` and live generation need it.
7. Deploys Helm with `attachedServices.comms-factory.enabled=true`, `COMMS_FACTORY_BASE_URL=http://centaur-centaur-attached-comms-factory:8080`, and local default harness `claude-code` so Slack turns use Anthropic rather than Codex.
