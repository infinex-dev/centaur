...
PR head:   0c7f64a56cfa15b30570fbf52847e0342df61df3
...
4. Clones/checks out comms-factory at PR head `0c7f64a56cfa15b30570fbf52847e0342df61df3` unless you pass `--comms-factory-repo PATH` or `--comms-factory-ref REF`.
5. Builds/imports `comms-factory-api:0c7f64a56cfa15b30570fbf52847e0342df61df3`.
...
Grounding (direction 2: comms-factory calling Centaur's native tools) uses the
tool-plane-native `/ground` contract:

- Wire envelope: `schema_version: comms_factory.ground_from_tools.v1`,
  `mode: ground_from_tools`, and a `tool_plane` reference object. The legacy
  `comms_factory.ground_from_capabilities.v1` / `capability_plane` names are gone
  on the Centaur side.
- The Centaur `comms_factory` tool client is the single authority for the
  `tool_plane` reference: it derives `base_url`/`tools_url`/`auth`
  (`{type: bearer_env, env: CENTAUR_TOKEN}`) from deployment env so a tool caller
  cannot redirect the scoped token, and merges the workflow-supplied
  `idempotency_prefix`. The workflow's `tool_plane_ref` contributes only that
  prefix (or `None` to early-block when the tool plane is unconfigured).
- comms-factory image `0c7f64a` is **dual-accept**: it serves the new contract
  and still accepts the legacy `capability_plane` names for one release so a
  non-atomic rolling restart can't drop grounding requests. **Deferred follow-up:**
  remove that legacy-acceptance branch from comms-factory once the pinned Centaur
  image is confirmed on the tool-plane contract.
- `comms_factory.ground` (the unused plain method) was removed; only
  `ground_from_tools` remains. The `"capability"` *fact category* is unrelated to
  the capability *plane* and is unchanged.
