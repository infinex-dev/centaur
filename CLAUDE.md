...
- **Disk pressure is real.** The VM is ~80% full of unrelated rootless podman storage (don't delete it). The 5.9GB `centaur-agent` image can trip the kubelet disk-pressure taint and get GC'd → grow the disk first. **`podman machine set --disk-size N` only grows the *virtual* disk; the guest partition/FS does NOT auto-grow** — after a stop/set/start, finish online inside the VM: `podman machine ssh "sudo growpart /dev/vda 4 && sudo xfs_growfs /"` (verify with `df -h /`). The host Mac's own free space is the real ceiling (the VM raw file grows as the guest fills). api+slackbot+iron-proxy fit fine; the media-renderer/runstore images need the headroom.
- **k3s ENFORCES NetworkPolicy** by default (kind doesn't; **whether the prod deploy-box CNI enforces is UNVERIFIED** — origin §8 / the video plan's U1 flag it as a required audit, and the media-renderer air-gap posture depends on the answer, so don't assume prod behaves like kind). The API unconditionally creates a per-sandbox egress policy allowing only API:8000 + iron-proxy:8080 — no port 53, so sandboxes can't reach CoreDNS and DNS-dependent harnesses (e.g. codex) fail with `EAI_AGAIN`. **Local steady state: enforcement is OFF** — `contrib/scripts/k3s-local.sh` idempotently writes `disable-network-policy: true` to `/etc/rancher/k3s/config.yaml` in the VM. To spike with enforcement ON (renderer/runstore air-gap checks), `sed -i '/disable-network-policy/d' …` + `systemctl restart k3s`, run the check (sandbox DNS breaks during the window), then re-run `k3s-local.sh` to restore. See `docs/runbooks/video-capacity-audit.md`.
