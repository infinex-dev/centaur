...
COMMS_FACTORY_REF="${COMMS_FACTORY_REF:-4d2c164c4db3eb4e549552aabe9a8c3820e663b9}"
...
attachedServices:
  comms-factory:
    ...
    env:
      ...
      # research calls. Keep this above the comms-factory client's 20s default
...
api:
  ...
  # First-dogfood tool set. Grounding runs on repo_context + websearch (Exa);
  # the comms_factory overlay tool is enabled here but stays overlay-only.
  # twitter is intentionally NOT enabled: this deploy wires no X/twitter
  # upstream credentials, and the first dogfood grounds on repo + web evidence.
  # The research scope bundle still authorizes tools:twitter, so enabling it
  # later is a one-line add here (no key change). Until then, comms-factory's
  # infinex_search_recent_posts (-> twitter.search_tweets) and the unmapped
  # fetch_json_api degrade to UNAVAILABLE gracefully.
  enabledTools:
    - comms_factory
    - repo_context
    - websearch
